...
def save_knowledge(
    topic: str, content: str, questions: str, added_by: str
) -> None:
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute(
...
def get_knowledge() -> List[Dict]:
    ...
    c = conn.cursor()
    c.execute(
        "SELECT topic, content, questions FROM knowledge_base ORDER BY created_at DESC"
    )
    rows = c.fetchall()
    conn.close()
...
def get_knowledge_by_topic(topic: str) -> Optional[Dict]:
    ...
    c = conn.cursor()
    c.execute(
        "SELECT topic, content, questions FROM knowledge_base WHERE topic = ?",
        (topic,),
    )
    row = c.fetchone()
    conn.close()
...
def create_logs_xlsx(rows: List[Tuple], sheet_name: str = "logs") -> bytes:
    """Генерирует минимально валидный .xlsx из списка кортежей через zipfile + xml."""
    import zipfile
    ...
    sheets = ET.SubElement(wb, f"{{{ns}}}sheets")
    ET.SubElement(
        sheets,
        f"{{{ns}}}sheet",
        {
            f"{{{ns}}}name": sheet_name,
            f"{{{ns}}}sheetId": "1",
            f"{{{rns}}}id": "rId1",
        },
    )
    ...
    for r_idx, row in enumerate(rows, 1):
        ...
        for c_idx, value in enumerate(row):
            ...
            col_letter = c_idx
            if c_idx >= 26:
                col_prefix = chr(65 + c_idx // 26 - 1)
                col_letter = c_idx % 26
            cell_ref = f"{col_prefix}{chr(65 + col_letter)}{r_idx}"
            cell = ET.SubElement(row_elem, f"{{{ns}}}c", {f"{{{ns}}}r": cell_ref})
            v = ET.SubElement(cell, f"{{{ns}}}v")
            v.text = str(value) if value is not None else ""

    ws_xml = ET.tostring(root, encoding="unicode", xml_declaration=True)
    ws_xml = ws_xml.replace("ns0:", "").replace("xmlns:ns0=", "xmlns=")
    ws_xml = ws_xml.encode("utf-8")
