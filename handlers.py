...
async def handle_text(message: Message):
    ...
    if codes and found_codes and not is_calc:
        ...
        radio = (
            "\n⚡ Радиосбор: 73 860 ₽ (фикс)"
            if any(is_radio_electronics(c) for c in codes)
            else ""
        )
        # Таможенный сбор по шкале ПП РФ №1637
        fee_scale = (
            "Таможенный сбор: от 1 231 ₽ до 73 860 ₽ (зависит от ТС)\n"
            "  до 200 тыс→1 231 ₽ | 200-450 тыс→2 462 ₽ | 450 тыс-1,2 млн→4 924 ₽\n"
            "  1,2-2,7 млн→13 541 ₽ | 2,7-4,2 млн→18 465 ₽ | 4,2-5,5 млн→21 344 ₽\n"
            "  5,5-10 млн→49 240 ₽ | свыше 10 млн→73 860 ₽"
        )
        await message.answer(
