...
async def handle_text(message: Message):
    ...
    if not found_codes and not codes:
        ...
        # Если НЕТ ВЭД-интента — пропускаем весь блок поиска → AI-ассистент
        has_ved_intent = any(kw in text_lower for kw in VED_INTENT_KEYWORDS)
        if not has_ved_intent:
            # Блок поиска пропущен — код продолжится ниже к AI-ассистенту (сценарий 3)
            pass
        else:
            # === БЛОК ПОИСКА ПО МАТЕРИАЛАМ (только при has_ved_intent=True) ===
        ...
        # === Весь блок поиска только при has_ved_intent=True ===
        if has_ved_intent:
