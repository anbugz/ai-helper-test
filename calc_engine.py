...
def _strip_ai_assistant_junk(answer: str) -> str:
    ...
    for line in lines:
        ...
        # Начало блока "Пример", "Допустим", "Что нужно уточнить" — пропускаем
        if any(kw in ls.lower() for kw in (
            ...
            "инвойс: **", "фрахт: **", "tc:", "тс: **",
            "что нужно уточнить", "что уточнить",
            "направление и схема", "схема работы",
            "рекомендации", "советы по оптимизации",
            "для точного расчёта", "если уточнишь",
        )):
            skip_section = True
            continue
        ...
        # Убираем markdown-таблицы (строки с |)
